...
async function main() {
  ...
  const steps: [string, () => Promise<{ count: number }>, () => Promise<number>][] = [
    ...
    // These two do not cascade from Opportunity — remove them by opportunity ID first.
    ['pricing workspaces on fixture opportunities (cascades scenarios/reviews)',
      () => prisma.pricingWorkspace.deleteMany({ where: { opportunityId: { in: FIXTURE_OPPORTUNITY_IDS } } }),
      () => prisma.pricingWorkspace.count({ where: { opportunityId: { in: FIXTURE_OPPORTUNITY_IDS } } })],
    ['proposal submissions on fixture opportunities (cascades checklist/status history/validation checks)',
      () => prisma.proposalSubmission.deleteMany({ where: { opportunityId: { in: FIXTURE_OPPORTUNITY_IDS } } }),
      () => prisma.proposalSubmission.count({ where: { opportunityId: { in: FIXTURE_OPPORTUNITY_IDS } } })],
    ['fixture opportunities (cascades pursuits/proposals/compliance matrices/matches/milestones)',
