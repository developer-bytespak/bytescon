...
<#
.SYNOPSIS
  Pre-demo smoke test for opportunity-mcp. Exercises auth -> tools/list ->
  tools/call -> audit-write end-to-end via JSON-RPC over stdio.

.DESCRIPTION
  Spawns `node dist/server.js` as a child process, drives the MCP
  handshake, calls search_opportunities, prints raw JSON-RPC responses
  and captured stderr. Re-runnable in <5s; use as the canonical
  pre-VETS26 health check.

.PARAMETER Keyword
  Search keyword to test with. Defaults to "Veterans" which matches
  seeded demo opps 001 + 002.
...
.EXAMPLE
  ./scripts/smoke.ps1
  ./scripts/smoke.ps1 -Keyword "Cybersecurity"
#>

param(
    [string]$Keyword = "Veterans",
...
Read-Response "TOOLS/CALL RESPONSE" 10000
